...
FolderPath = "C:\\Workbench\\Computer Science\\Python Programming\\Machine Learning And Chemistry\\Machine Learning And Chemistry\\TaskData4\\"
#pipe = PL([("SVC", SVC(gamma="scale", random_state=1551))])
#param_grid = {
...
#    "SVC__degree" : [2, 3],
#}
#grid = GridCV(pipe, param_grid, cv=5)
pipe = PL([("SVC", SVC(cache_size=1024, C=10.0, kernel="rbf", degree=5, gamma="scale", tol=0.0001, random_state=1551))])
import sys
...
def ReadData():
    ...
    TestLabels.shape
    scaler = StdScaler()
    TrainImages = scaler.fit_transform(TrainImages)
    TestImages = scaler.fit_transform(TestImages)    


def ModelTraining():
#    global grid, TrainImages, TrainLabels
#    grid.fit(TrainImages, TrainLabels)
...
#    ModelScore = grid.score(TestImages, TestLabels)
    ...
    OutputName = FolderPath + "dSVMReport4.dat"
...
#    JL.dump(grid, FolderPath + "dSVM4.dump")
    JL.dump(pipe, FolderPath + "dSVM4.dump")
